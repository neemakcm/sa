...
$(function() { $.ajaxSetup({ headers: { "X-CSRF-TOKEN": $('meta[name="csrf-token"]').attr("content") } }), $("#category_id").on("change", function(e) { var t = $(this).val();
        $.ajax({ type: "POST", url: base_url + "/escalate-to-service/category-product", data: { id: t }, success: function(e) { var t = JSON.parse(e);
                console.log(t), t ? ($("#product_id").empty(), $("#product_id").focus, $("#product_id").append('<option value="">  Select  Product </option>'), $.each(t, function(e, t) { $('select[name="product_id"]').append('<option value="' + t.id + '">' + t.sku + "</option>") })) : $("#product_id").empty() } }) }), $("#product_id").on("change", function(e) { var t = $(this).val();
